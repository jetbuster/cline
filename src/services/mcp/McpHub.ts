...
export class McpHub {
	...
	public async registerMcpServerDynamically(config: {
		...
		try {
			...
			console.error("Failed to register MCP server dynamically:", error)
			throw error
		}
	}
	...
	public async unregisterMcpServerDynamically(serverName: string): Promise<McpServer[]> {
		try {
			...
			await this.deleteConnection(serverName)
			console.log(`Unregistered transient MCP server: ${serverName}`)
			...
			console.error("Failed to unregister MCP server dynamically:", error)
			throw error
		}
	}
